# Создайте систему управления банковскими счетами, которая позволяет создавать, управлять и выполнять операции
...
# balance (баланс счета) и client (объект типа Client, которому принадлежит счет). Класс также должен иметь методы deposit
# (amount) и withdraw(amount), которые позволяют пополнить или снять деньги со счета.
...
class BankAccount:
    ...
    def deposit(self, amount):
        self.balance += amount
        print(f'Вы положили на счет: {amount}')
...
class Bank:
    ...
    def create_account(self, client, initial_balance):
        ...
        account = BankAccount(account_number, client, initial_balance)
    ...
    def get_account(self, account_number):
        if account_number in self.accounts:
            ...
            return 'Такого счета не существвует'
